...
export default function SalesInvoices() {
  ...
  // Estados válidos para sales_invoices: borrador,emitida,pagada,anulada
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const [invoiceRes, customerRes] = await Promise.all([client.get('/sales_invoices'), client.get('/customers')]);
  ...
  const filtered = useMemo(() => {
    ...
    return invoices.filter((invoice) => {
      const customerName = customers.find((customer) => customer.id === invoice.customer_id)?.name || '';
      return (
        invoice.invoice_number?.toLowerCase().includes(value) ||
        customerName.toLowerCase().includes(value) ||
        String(invoice.customer_id).includes(value)
      );
    });
  }, [customers, invoices, search]);

  const getCustomerName = (id) => customers.find((customer) => customer.id === id)?.name || `Cliente ${id}`;
  ...
  const toggleExpand = async (id) => {
    ...
    if (!details[id]) {
      try {
        const response = await client.get(`/sales_invoices/${id}`);
  ...
  const handleCreate = async (event) => {
    ...
    try {
      await client.post('/sales_invoices', form);
      showToast('Factura de venta creada');
      setShowModal(false);
      setForm({ invoice_number: '', customer_id: '', invoice_date: '', total_amount: '', status: 'borrador' });
      fetchData();
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <h1 className="text-2xl font-black tracking-tight text-white sm:text-3xl">Facturas de Venta</h1>
          <p className="mt-1 text-sm text-slate-400">Listado y trazabilidad de facturacion a clientes.</p>
        </div>
      ...
      <section className="erp-panel overflow-hidden">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-white/10 px-4 py-3">
          <label className="relative w-full max-w-sm">
            ...
            <input className="erp-input pl-9" placeholder="Buscar por numero o cliente..." value={search} onChange={(event) => setSearch(event.target.value)} />
          </label>
          ...
          <table className="min-w-full">
            <thead className="bg-surface-800/40">
              <tr>
                ...
                <th className="erp-th">Cliente</th>
                ...
                filtered.map((invoice) => (
                  <Fragment key={invoice.id}>
                    <tr className="cursor-pointer transition hover:bg-white/5" onClick={() => toggleExpand(invoice.id)}>
                      ...
                      <td className="erp-td">{getCustomerName(invoice.customer_id)}</td>
        ...
        <Modal
          ...
          footer={
            <>
              ...
              <button type="submit" form="sales-form" className="erp-btn-primary" disabled={saving}>
                {saving ? 'Guardando...' : 'Crear factura'}
              </button>
            </>
          }
        >
          <form id="sales-form" className="grid grid-cols-1 gap-4 sm:grid-cols-2" onSubmit={handleCreate}>
            <label>
              <span className="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-slate-400">Numero</span>
              <input className="erp-input" value={form.invoice_number} onChange={(event) => setForm({ ...form, invoice_number: event.target.value })} required placeholder="FV-2026-001" />
            </label>
            <label>
              <span className="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-slate-400">Cliente</span>
              <select className="erp-input" value={form.customer_id} onChange={(event) => setForm({ ...form, customer_id: event.target.value })} required>
